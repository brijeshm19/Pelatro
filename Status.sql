...
CASE 
				when MONTH(CUST_DOB::DATE)=MONTH((date('"+context.vLoadDate+"')-1)::DATE) and DAY(CUST_DOB::DATE)=DAY((date('"+context.vLoadDate+"')-1)::DATE) and WEEK (CUST_DOB::DATE)=WEEK((date('"+context.vLoadDate+"')-1)::DATE) then 'Today'
				when MONTH(CUST_DOB::DATE)=MONTH((date('"+context.vLoadDate+"')-1)::DATE) and WEEK (CUST_DOB::DATE)=WEEK((date('"+context.vLoadDate+"')-1)::DATE) then 'Current Week'
				when MONTH(CUST_DOB::DATE)=MONTH((date('"+context.vLoadDate+"')-1)::DATE) and WEEK (CUST_DOB::DATE)=WEEK((date('"+context.vLoadDate+"')-1)::DATE)+1 then 'Upcoming Week'
				when MONTH(CUST_DOB::DATE)=MONTH((date('"+context.vLoadDate+"')-1)::DATE) and WEEK (CUST_DOB::DATE)!=WEEK((date('"+context.vLoadDate+"')-1)::DATE) then 'Current Month'
				when MONTH(CUST_DOB::DATE)=MONTH((date('"+context.vLoadDate+"')-1)::DATE)+1 then 'Next Month'
				else 'Later'
			END as CUSTOMER_DOB,
a.CUST_LAST_PRODUCT_NAME as USER_TYPE,
DATEDIFF(day,cast(a.FULL_ACTIVATION_DATE as date), date('"+context.vLoadDate+"')-1)as DAYS_SINCE_REGISTRATION,
...
DATEDIFF(day, cast(a.CUST_REGISTRATION_DATE as date), date('"+context.vLoadDate+"')-1)as EMAIL_REGISTRATION_DATE,
e.MAX_ACCOUNTS_LIMIT_FLAG,
f.score as CUSTOMER_SENTIMENT_SCORE,
f.score as CUSTOMER_MOOD_SCORE_CSAT,
case when MONTH(CUST_DOB::DATE)=MONTH((date('"+context.vLoadDate+"')-1)::DATE) and DAY(CUST_DOB::DATE)=DAY((date('"+context.vLoadDate+"')-1)::DATE) then 1 else 0 end as NETWORK_ANNIVERSARY_DAY,
...
SELECT 
  ...
  where  snapshotdate = date('"+context.vLoadDate+"')-1 and subscriptiontype NOT IN ('VM_AppleWatch','VM_EventSims'))x
  ) a left join ( 
 SELECT 
   accountnum AS IN_SNAP_ACCOUNT_NUMBER,
...
select
		...
		from
			(
				...
				where
					snapshotdate >= date('"+context.vLoadDate+"')-91
								...
								SELECT
									*,
									...
									WHERE
										accountnum>1
										and tstamp< cast(
											date('"+context.vLoadDate+"') as date
										)
							)b
						where
...
left join (select * from ae_prod.customer_master where REGISTRATION_ID not like '') a on (x.IN_ACCOUNT_NUMBER=a.IN_ACCOUNT_NUMBER)
left join (select 
...
WHERE accountnum>1
and tstamp< cast(date('"+context.vLoadDate+"') as date))b
...
left join (select 
accountnum as IN_ACCOUNT_NUMBER, 
DATEDIFF(day, tstamp , date('"+context.vLoadDate+"')) as DAYS_SINCE_PORTOUT
...
left join (select score	,UID from (SELECT
score,
regexp_replace(extra_info , '[^0-9]', '') AS UID,row_number()over(partition by regexp_replace(extra_info , '[^0-9]', '') order by cast(a._date as timestamp) desc) rnk
FROM
api.usabilla_data a
left join 
api.usabilla_campaigns b 
on b.campaign_id =a.campaign_id
where LENGTH(regexp_replace(extra_info , '[^0-9]', ''))>1
)x
where rnk=1) f on (x.IN_ACCOUNT_NUMBER=f.UID)
)y
;