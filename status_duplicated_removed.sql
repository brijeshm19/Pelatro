select IN_ACCOUNT_NUMBER, count(*) from (
select 
IN_ACCOUNT_NUMBER,
CUSTOMER_STATUS,
LAST_ACTIVITY_DATE,
...
score,
regexp_replace(extra_info , '[^0-9]', '') AS UID,row_number()over(partition by regexp_replace(extra_info , '[^0-9]', '') order by cast("_date" as timestamp) desc) rnk
FROM
api.usabilla_data a
left join 
