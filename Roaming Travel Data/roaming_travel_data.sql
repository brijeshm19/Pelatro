...
SELECT  
*,
CASE WHEN CAST(DATETIME  AS DATE)>=ADD_MONTHS(date('"+context.vLoadDate+"')-1 ,-1) THEN 1 ELSE 0 END AS FLAG_RLH_1M,
CASE WHEN CAST(DATETIME  AS DATE)>=ADD_MONTHS(date('"+context.vLoadDate+"')-1 ,-3) THEN 1 ELSE 0 END AS FLAG_RLH_3M
FROM(
SELECT *,
...
WHERE CAST(DATETIME  AS DATE)>=ADD_MONTHS(date('"+context.vLoadDate+"') -1 ,-6) AND CAST(DATETIME  AS DATE) < date('"+context.vLoadDate+"')  AND regexp_like(a.MCC,'[0-9]') AND a.MCC NOT IN ('000','901')
ORDER BY MSISDN,ID
)A 
)B
