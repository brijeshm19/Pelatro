select 
in_account_number,
sum(total_volume_data) as DATA_USAGE_30_days,
sum(total_volume_data/30) as DATA_USAGE_30_DAY_AVG,
case 
...
end as Last_30_days_data_usage,
sum(total_national_minutes_outgoing) as Last_30_days_outgoing_national_voiceminutes,
sum(total_international_minutes_outgoing) as Last_30_days_outgoing_international_voiceminutes,
sum(total_roaming_minutes_outgoing) as Last_30_days_outgoing_roaming_voiceminutes,
sum(total_roaming_minutes_incoming) as Last_30_days_incoming_roaming_voiceminutes,
	sum(total_national_minutes_outgoing + total_national_minutes_incoming) as NATIONAL_VOICE_USAGE_30_days, 
	sum(total_international_minutes_outgoing + total_international_minutes_incoming) as INTERNATIONAL_VOICE_USAGE_30_days,
	sum(ad_total_revenue_net/30) as Last_30_days_ARPU,
sum(total_national_minutes_outgoing+total_international_minutes_outgoing+total_roaming_minutes_outgoing) as total_minutes_outgoing_30_days
	from (
SELECT
	ad_date,
	in_account_number,
...
FROM
 ...
 -- count(distinct case when fus_service_type_id in (1,2) and fus_call_direction_id = 1 then fus_bno else 0 end) as UNIQUE_NUMBERS_CALLED_TEXTED_TO
	...
	where
		...
		and date(ad_date) >= date('"+context.vLoadDate+"')-30
