select 
in_account_number,
sum(total_volume_data) as DATA_USAGE_M_2,
	sum(total_national_minutes_outgoing + total_national_minutes_incoming) as NATIONAL_VOICE_USAGE_M_2, 
	sum(total_international_minutes_outgoing + total_international_minutes_incoming) as INTERNATIONAL_VOICE_USAGE_M_2,
	sum(ad_total_revenue_net/30) as ARPU_M_2
	from (
SELECT
	ad_date,
	in_account_number,
...
FROM
 ...
 -- count(distinct case when fus_service_type_id in (1,2) and fus_call_direction_id = 1 then fus_bno else 0 end) as UNIQUE_NUMBERS_CALLED_TEXTED_TO
	...
	where
		to_char(date(ad_date), 'yyyy-mm') in (select to_char(ADD_MONTHS(date('"+context.vLoadDate+"'),-2),'yyyy-mm')) 
