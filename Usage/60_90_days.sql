select 
in_account_number,
sum(total_volume_data) as DATA_USAGE_60_90_days,
	sum(total_national_minutes_outgoing + total_national_minutes_incoming) as NATIONAL_VOICE_USAGE_60_90_days, 
	sum(total_international_minutes_outgoing + total_international_minutes_incoming) as INTERNATIONAL_VOICE_USAGE_60_90_days,
	sum(ad_total_revenue_net/30) as Last_60_90_days_ARPU,
	sum(total_national_minutes_outgoing) as Last_60_90_days_outgoing_national_voiceminutes
	from (
SELECT
	ad_date,
	in_account_number,
...
FROM
 ...
 -- count(distinct case when fus_service_type_id in (1,2) and fus_call_direction_id = 1 then fus_bno else 0 end) as UNIQUE_NUMBERS_CALLED_TEXTED_TO
	...
	where
		date(ad_date) <= date('"+context.vLoadDate+"')-60
		and date(ad_date) >=  date('"+context.vLoadDate+"')-90
...
group by
	1,
	2 )t 
/*where in_account_number in 
(select IN_ACCOUNT_NUMBER from ae_prod.customer_master 
where CUST_DAYS_SINCE_LAST_ACTIVITY not like '180+'
and to_number(CUST_DAYS_SINCE_LAST_ACTIVITY) <= 360)*/
group by 1
;	