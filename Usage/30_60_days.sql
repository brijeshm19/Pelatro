select 
in_account_number,
sum(total_volume_data) as DATA_USAGE_30_60_days,
	sum(total_national_minutes_outgoing + total_national_minutes_incoming) as NATIONAL_VOICE_USAGE_30_60_days, 
	sum(total_international_minutes_outgoing + total_international_minutes_incoming) as INTERNATIONAL_VOICE_USAGE_30_60_days,
	sum(ad_total_revenue_net/30) as Last_30_60_days_ARPU,
	sum(total_national_minutes_outgoing) as Last_30_60_days_outgoing_national_voiceminutes,
    sum(total_international_minutes_outgoing) as Last_30_60_days_outgoing_international_voiceminutes,
sum(total_national_minutes_outgoing+total_international_minutes_outgoing+total_roaming_minutes_outgoing) as total_minutes_outgoing_30_60_days
	from (
SELECT
	ad_date,
	in_account_number,
...
FROM
 ...
 -- count(distinct case when fus_service_type_id in (1,2) and fus_call_direction_id = 1 then fus_bno else 0 end) as UNIQUE_NUMBERS_CALLED_TEXTED_TO
	...
	where
		date(ad_date) <= date('"+context.vLoadDate+"')-30
		and date(ad_date) >= date('"+context.vLoadDate+"') -60
